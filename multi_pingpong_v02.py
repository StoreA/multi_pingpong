...
- Traceroute support with --traceroute flag, displayed in ping window.

Usage:
    ./ping_monitor.py 8.8.8.8 google.com
    ./ping_monitor.py -f troubleshoot_ips.txt
    ./ping_monitor.py --traceroute 8.8.8.8
    ./ping_monitor.py --traceroute -f my_ips.txt
...
def parse_arguments():
    ...
    parser.add_argument("--traceroute", action="store_true", help="Run traceroute on the specified IPs")
...
async def traceroute_ip(ip):
    """Run traceroute on an IP and return the output.

    Args:
        ip (str): IP address or domain to traceroute.

    Returns:
        str: Traceroute output or error message.
    """
    cmd = 'tracert' if platform.system().lower() == 'windows' else 'traceroute'
    cmd = f"{cmd} -w 2 -q 1 {ip}"
    try:
        process = await asyncio.create_subprocess_shell(
            cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
        ...
        if stderr:
            error = stderr.decode()
            if error:
                return f"Traceroute error for {ip}: {error}"
        return output
    except asyncio.TimeoutError:
        return f"Traceroute timeout for {ip}"
    except Exception as e:
        return f"Error running traceroute for {ip}: {e}"
...
def print_ping_results(ping_data, histories, use_color, ip_mappings, traceroute_results=None):
    """Print a table with ping results, statistics, and optional traceroute output.
    ...
    Args:
        ...
        traceroute_results (dict, optional): Traceroute output for each IP.
    """
    clear_screen()
    ...
    if traceroute_results:
        print("\n")
        print(f"{YELLOW if use_color else ''}{BOLD if use_color else ''}{TRACEROUTE_ASCII.strip()}{RESET if use_color else ''}")
        print("-" * terminal_width)
        for original, resolved in ip_mappings:
            display_ip = original if original == resolved else f"{original} ({resolved})"
            print(f"Traceroute for {display_ip}:")
            print("-" * terminal_width)
            print(traceroute_results.get(original, "No traceroute data available.").strip())
            print("-" * terminal_width)
...
async def main():
    ...
    # Run traceroute if requested and store results
    traceroute_results = {}
    if args.traceroute:
        print("\nRunning traceroute for specified IPs...")
        for original, resolved in args.ip_mappings:
            result = await traceroute_ip(resolved)
            traceroute_results[original] = result
        print("Traceroute complete. Starting ping monitor...")

    print("\nInitializing ping monitor...")
    ...
    try:
        ...
        while args.count == 0 or ping_count < args.count:
            ...
            print_ping_results(ping_data_mapped, histories, not args.no_color, args.ip_mappings, traceroute_results)
            ping_count += 1
